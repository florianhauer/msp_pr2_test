...
State<2*AD> obstateState(const ob::State* s){
	State<2*AD> s2;
	for(int i=0;i<2*AD;++i){
...
bool isObstacle(const ob::State* state){
	...
	setState(*current_state,group_variable_values_r,group_variable_values_l);
...
int main(int argc, char **argv)
{
	...
	ob::RealVectorBounds bound(2*AD);
	...
	space.reset(new ob::RealVectorStateSpace(2*AD));
	...
	*/
	...
	if(solved==ompl::base::PlannerStatus::StatusType::EXACT_SOLUTION || solved==ompl::base::PlannerStatus::StatusType::APPROXIMATE_SOLUTION){
		...
		std::deque<State<2*AD>> sol;//current pose not included
